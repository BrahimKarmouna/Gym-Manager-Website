            ...
            <q-td key="actions" :props="props">
              <div class="flex gap-2">
                ...
                <q-btn
                  ...
                  color="secondary"
                  icon="lock"
                  @click="viewPermissions(props.row)"
                >
    ...
    <q-dialog v-model="showAssistantDialog" persistent maximized-mobile>
      <q-card class="max-w-2xl w-full">
        <q-card-section class="flex items-center justify-between">
          <h2 class="text-h6">
            {{ showEditModal ? "Edit Assistant" : "Add New Assistant" }}
          </h2>
          <q-btn
            icon="close"
            flat
          ...
          <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <q-select
                ...
                :rules="[
                  (val) => (val && val.length > 0) || 'Please select a role',
                ]"
                  ...
                  <q-item v-bind="scope.itemProps">
                    <q-item-section>
                      ...
                      <q-item-label caption>{{
                        scope.opt.description || "No description available"
                      }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </template>
              ...
              <div
                class="q-field q-field--outlined q-field--dense q-field--labeled q-field--no-input"
              >
                <div class="q-field__inner relative-position">
                  <div class="q-field__control relative-position row no-wrap">
                    <div
                      class="q-field__control-container col relative-position"
                    >
        ...
        <q-card-actions align="right" class="q-px-md q-pb-md">
          ...
          <q-btn
            ...
            :disable="
              ...
              !formData.email ||
              !formData.role ||
              (!showEditModal && !formData.password) ||
    ...
    <q-dialog v-model="showPermissionsModal" persistent maximized-mobile>
      <q-card class="max-w-3xl w-full">
        ...
        <q-card-section>
          ...
          <div v-else>
            <!-- Assistant info at the top of permissions modal -->
            <div class="flex items-center gap-4 mb-4 bg-gray-100 p-3 rounded">
              <q-avatar size="lg">
                <img
                  :src="
                    selectedAssistant?.photo
                      ? `/storage/${selectedAssistant.photo}`
                      : '/img/default-avatar.png'
                  "
              ...
              <div>
                <div class="text-h6">{{ selectedAssistant?.name }}</div>
                <div class="text-subtitle1">{{ selectedAssistant?.email }}</div>
                <q-badge
                  :color="getRoleBadgeColor(selectedAssistant?.role)"
                  text-color="white"
                  class="q-px-sm q-mt-sm"
                >
                  {{ getRoleLabel(selectedAssistant?.role) }}
                </q-badge>
              </div>
            </div>
                      ...
                      <q-card-section>
                        <div class="flex justify-between items-center q-mb-sm">
                          ...
                          <q-btn
                            ...
                            dense
                            :color="
                              isAllGroupSelected(group) ? 'negative' : 'primary'
                            "
                            :label="
                          ...
                          >
                            <div class="flex-1">
                              <div class="font-medium">
                                {{ formatPermissionName(perm.display) }}
                              </div>
  ...
  computed: {
    ...
    filteredAssistants() {
      ...
      if (this.roleFilter.value !== "all") {
        filtered = filtered.filter(
          (assistant) => assistant.role === this.roleFilter.value
        );
      }
      ...
      if (this.gymFilter.value !== "all") {
        filtered = filtered.filter(
          (assistant) => assistant.gym_id === this.gymFilter.value
        );
      }
  ...
  methods: {
    ...
    async fetchRoles() {
      try {
        const response = await axios.get(
          "/api/user-management/roles-permissions"
        );
        this.availableRoles = response.data.roles;

        // Update role options for filtering
        this.roleOptions = [
          { label: "All Roles", value: "all" },
    ...
    async saveAssistant() {
      try {
        ...
        const validRoles = this.availableRoles.map((role) => role.id);
        data.append(
          "role",
          validRoles.includes(this.formData.role)
            ? this.formData.role
            : this.availableRoles[0].id
        );
        ...
        if (this.showEditModal) {
          ...
          response = await axios.post(
            `/api/assistants/${this.selectedAssistant.id}`,
            data
          );
        } else {
    ...
    editAssistant(assistant) {
      ...
      this.formData = {
        ...
        gym_id:
          assistant.gym_id || (this.gyms.length > 0 ? this.gyms[0].id : null),
        active:
          assistant.active === undefined ? true : Boolean(assistant.active),
    ...
    async viewPermissions(assistant) {
      ...
      try {
        // Fetch assistant permissions using the correct API endpoint
        const response = await axios.get(
          `/api/assistants/${assistant.id}/permissions`
        );
        ...
        if (response.data && response.data.permissions) {
          ...
          this.permissionGroups = this.groupPermissionsByResource(
    ...
    async savePermissions() {
      ...
      try {
        ...
        await axios.put(
          `/api/assistants/${this.selectedAssistant.id}/permissions`,
          {
    ...
    async deleteAssistant() {
      ...
      try {
        ...
        this.assistants = this.assistants.filter(
          (a) => a.id !== this.selectedAssistant.id
        );
